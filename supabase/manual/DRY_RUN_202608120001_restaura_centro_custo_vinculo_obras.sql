...
language sql stable security definer set search_path=public,pg_temp as $$
 ...
 from privados p
 left join public.funcionarios f on f.id=p.funcionario_id and f.ativo and f.deleted_at is null and f.visivel_obras_control
 left join lateral(select a.obra_id from public.alocacoes a where a.funcionario_id=f.id order by a.data desc limit 1) atual on true
 left join public.obras o on o.id=atual.obra_id
 where auth.uid() is not null
 union all
 select f.id,f.nome,f.id,true,f.visivel_obras_control,f.visivel_passagens,f.escopo_passagens,atual.obra_id,o.nome
 from public.funcionarios f
 left join lateral(select a.obra_id from public.alocacoes a where a.funcionario_id=f.id order by a.data desc limit 1) atual on f.visivel_obras_control
 left join public.obras o on o.id=atual.obra_id
 where auth.uid() is not null and f.ativo and f.deleted_at is null and f.visivel_passagens
  ...
  and not (f.escopo_passagens='restrito_ro' and not f.visivel_obras_control and exists(select 1 from unicos u where u.legado_id=f.id))
 order by 2;
$$;
