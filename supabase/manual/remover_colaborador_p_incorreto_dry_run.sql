-- DRY-RUN: valida todas as guardas, exercita o DELETE e sempre desfaz.
begin;
...
begin
  ...
  if v_quantidade <> 1 then
    raise exception 'LIMPEZA_P_ABORTADA: esperado exatamente 1 cadastro P; encontrados %', v_quantidade;
  end if;
  if v_funcionario_id is not null then
    raise exception 'LIMPEZA_P_ABORTADA: cadastro P possui funcionario_id; alternativa segura: inativacao';
  end if;
  ...
  raise notice 'DRY-RUN APROVADO: cadastro P seria excluido; ROLLBACK obrigatorio a seguir';
end $$;
