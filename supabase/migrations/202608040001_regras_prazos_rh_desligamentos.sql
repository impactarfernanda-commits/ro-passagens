...
alter table public.ro_passagem_solicitacoes add constraint ro_desligamento_subtipo_ck check (
  ...
  (motivo='desligamento' and desligamento_subtipo in ('programado_outros','justa_causa','pedido_demissao','ma_conduta')) or
...
language sql stable security definer set search_path=public,pg_temp as $$
  select public.ro_is_rh_active() or public.ro_can_operate() or public.ro_role() in ('gerente','diretor')
...
declare v_role text:=public.ro_role(auth.uid()); v_rh boolean:=public.ro_is_rh_active(); v_reg record; v_min date; v_primeiro timestamptz; v_categoria text;
begin
  if auth.uid() is null or new.solicitante_id<>auth.uid() then raise exception 'SOLICITANTE_INVALIDO'; end if;
  if new.motivo='viagem_diretoria' then raise exception 'MOTIVO_NAO_DISPONIVEL'; end if;
  if v_role not in ('gerente','diretor') and v_rh and new.motivo not in ('admissao','desligamento','inicio_obra') then raise exception 'MOTIVO_NAO_PERMITIDO'; end if;
  if v_role not in ('gerente','diretor') and not v_rh and new.motivo='admissao' then raise exception 'MOTIVO_NAO_PERMITIDO'; end if;
  if new.motivo='desligamento' and new.desligamento_subtipo not in ('programado_outros','justa_causa','pedido_demissao','ma_conduta') then raise exception 'SUBTIPO_DESLIGAMENTO_OBRIGATORIO'; end if;
  ...
  if tg_op='INSERT' then new.origem_solicitacao:=case when new.motivo is null then 'administrativo' when v_role in ('gerente','diretor') then 'gerencial' when v_rh then 'rh' else 'comum' end;
  ...
  if new.data_ida<v_min then
    if v_role not in ('gerente','diretor') then raise exception 'FORA_DO_PRAZO:%',v_min; end if;
...
begin
  for v_doc in select value from jsonb_array_elements(coalesce(p_documentos,'[]'::jsonb)) loop
    select * into v_obj from storage.objects o where o.bucket_id='ro-documentos-internos' and o.name=v_doc->>'storage_path' and o.owner_id=auth.uid();
...
begin v_ano:=extract(year from case when tg_op='DELETE' then old.data else new.data end); insert into public.ro_calendario_anos(ano,completo) values(v_ano,false) on conflict(ano) do update set completo=false,validado_em=null,validado_por=null; insert into public.ro_auditoria_interna(evento,detalhes,criado_por) values('calendario_alterado',jsonb_build_object('ano',v_ano,'operacao',tg_op),auth.uid()); if tg_op='DELETE' then return old; end if; return new; end $$;
...
drop policy if exists ro_internal_storage_insert on storage.objects; create policy ro_internal_storage_insert on storage.objects for insert to authenticated with check(bucket_id='ro-documentos-internos' and owner_id=auth.uid() and lower(storage.extension(name))='pdf');
