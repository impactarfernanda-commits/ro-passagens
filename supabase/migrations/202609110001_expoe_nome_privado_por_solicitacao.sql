...
set search_path = ''
as $$
  select s.id,coalesce(e.nome,f.nome)
  ...
  where auth.uid() is not null
    ...
    and (
      s.excluida_em is null
      or (s.excluida_em is not null and public.ro_can_operate())
    )
    and coalesce(nullif(btrim(e.nome),''),nullif(btrim(f.nome),'')) is not null;
$$;
...
as $$
  select e.id,e.nome,e.cidade,e.uf
  from public.ro_funcionarios_enderecos_privados e
  where e.id=p_colaborador_id
    and public.ro_can_view_private_addresses();
$$;
...
revoke all on function public.ro_nomes_colaboradores_solicitacoes(uuid[]) from public,anon;
revoke all on function public.ro_obter_colaborador_resumo(uuid) from public,anon;
revoke all on function public.ro_catalogo_colaboradores_viagem() from public,anon;
grant execute on function public.ro_nomes_colaboradores_solicitacoes(uuid[]) to authenticated,service_role;
grant execute on function public.ro_obter_colaborador_resumo(uuid) to authenticated,service_role;
