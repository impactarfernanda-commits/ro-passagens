create table public.portal_sso_handoffs (
  ...
  target_app text not null check (target_app = 'obras-control'),
  ...
  expires_at timestamptz not null check (expires_at > created_at and expires_at <= created_at + interval '120 seconds'),
...
returns table(user_id uuid, return_path text)
language sql
security definer
set search_path = public, pg_temp
as $$
  update public.portal_sso_handoffs
     set consumed_at = now()
...
create or replace function public.portal_limpar_sso_handoffs()
returns bigint language sql security definer set search_path = public, pg_temp as $$
  with removed as (
    delete from public.portal_sso_handoffs where expires_at < now() - interval '1 day' returning 1
