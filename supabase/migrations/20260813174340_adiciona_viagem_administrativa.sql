...
alter table public.ro_passagem_solicitacoes
  drop constraint if exists ro_motivo_valido;

alter table public.ro_passagem_solicitacoes
  add constraint ro_motivo_valido check (motivo is null or motivo in (
    'ferias','folga_campo','desligamento','transferencia_obra','viagem_diretoria',
    'admissao','inicio_obra','retorno_obra','recesso','viagem_administrativa'
  ));
...
language sql immutable set search_path=public,pg_temp as $$
 ...
 case when p_motivo='viagem_administrativa' then 'sem_prazo_minimo'
 ...
 case when p_motivo='viagem_administrativa' then 0
