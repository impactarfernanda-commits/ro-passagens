...
export function estadoEfetivoFolga(ciclo?:CicloFolga|null){
  if(ciclo?.solicitacao_futura_status==="recusada")return "Recusada";
  if(ciclo?.solicitacao_futura_status==="cancelada")return "Cancelada";
  if(ciclo?.solicitacao_futura_aprovacao_status==="reprovada")return "Reprovada na aprovação";
  return null;
}
