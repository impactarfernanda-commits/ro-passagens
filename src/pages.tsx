...
import readXlsxFile, { readSheet, type SheetData } from "read-excel-file/browser";
import {
  ArrowLeft,
  CheckCircle2,
  ExternalLink,
...
export function NovaSolicitacao({ userId, access }: { userId: string; access: Access }) {
  ...
  useEffect(()=>{
    ...
    const obrasId=funcionarios.find(x=>x.id===form.funcionario_id)?.funcionario_id;
    if(form.motivo!=="folga_campo"||!obrasId)return;
    setCicloLoading(true);
    supabase.rpc("ro_obter_ciclo_folga_funcionario",{p_funcionario_id:obrasId}).then(({data,error})=>{setCicloFolga(error?null:data as CicloFolga);if(error)setErro("Não foi possível consultar o ciclo de folga de campo.");setCicloLoading(false);});
...
type AddressEmployee={id:string;funcionario_id:string|null;nome:string;data_nascimento:string|null;cpf:string|null;rg:string|null;telefone:string|null;logradouro:string|null;bairro:string|null;cidade:string|null;uf:string|null;ativo:boolean;atualizado_em:string|null};
type ObrasMatchEmployee={id:string;nome:string;ativo:boolean;colaborador_id:string|null};
type AddressPreview={linha:number;colaborador_id:string|null;funcionario_id:string|null;nome:string;data_nascimento:string|null;cpf:string;rg:string;telefone:string;logradouro:string;bairro:string;cidade:string;estado:string;status:"novo_vinculado"|"novo_externo"|"atualizacao"|"possivel_duplicidade"|"conflito"|"erro"|"ignorado";mensagem:string;sugestoes:CollaboratorSuggestion[];permite_externo:boolean;resolucao_manual:boolean};
const addressLabels:Record<AddressField,string>={nome:"Nome",data_nascimento:"Data de nascimento",cpf:"CPF",rg:"RG",telefone:"Telefone",cep:"CEP",logradouro:"Logradouro",numero:"Número",complemento:"Complemento",bairro:"Bairro",cidade:"Cidade",uf:"UF",estado:"Estado"};

export function EnderecosFuncionarios({embedded=false}:{embedded?:boolean}={}) {
  // Substitui com compatibilidade funcional o antigo ro_salvar_endereco_funcionario.
  const [employees,setEmployees]=useState<AddressEmployee[]>([]); const [obrasEmployees,setObrasEmployees]=useState<ObrasMatchEmployee[]>([]); const [rows,setRows]=useState<SpreadsheetRows>([]);
  const [mapping,setMapping]=useState<ColumnMapping>({}); const [preview,setPreview]=useState<AddressPreview[]>([]);
  const [fileName,setFileName]=useState(""); const [confirmed,setConfirmed]=useState(false); const [message,setMessage]=useState("");const [reading,setReading]=useState(false);
  const [editorMessage,setEditorMessage]=useState<{kind:"success"|"error";text:string}|null>(null);const [savingEmployee,setSavingEmployee]=useState(false);
  const [search,setSearch]=useState("");const [selectedEmployee,setSelectedEmployee]=useState<AddressEmployee|null>(null);const [editingAddress,setEditingAddress]=useState({id:"",funcionario_id:"",nome:"",data_nascimento:"",cpf:"",rg:"",telefone:"",logradouro:"",bairro:"",cidadeUf:"",ativo:true});
  const load=useCallback(async()=>{const [privados,obras]=await Promise.all([supabase.rpc("ro_catalogo_colaboradores_privado",{p_busca:null}),supabase.rpc("ro_catalogo_funcionarios_obras_matching_rh")]);const error=privados.error||obras.error;if(error)setMessage(error.message);else{setEmployees((privados.data||[]) as AddressEmployee[]);setObrasEmployees((obras.data||[]) as ObrasMatchEmployee[])}},[]);
  useEffect(()=>{load()},[load]);
  async function choose(file?:File){setMessage("");setRows([]);setMapping({});setPreview([]);setConfirmed(false);setFileName("");if(!file)return;if(file.size>MAX_RH_XLSX_BYTES){setMessage("Arquivo muito grande. O limite para importação é 10 MB.");return}setReading(true);try{const data:SheetData=await readSheet(file);if(!isSpreadsheetRows(data)){setMessage("Não foi possível identificar uma tabela válida na primeira aba da planilha.");return}setRows(data);setMapping(autoMapHeaders(data[0]));setFileName(file.name)}catch(error){const detail=error instanceof Error?{name:error.name,message:error.message,etapa:"leitura_xlsx_rh"}:{name:"UnknownError",message:"Falha sem detalhes",etapa:"leitura_xlsx_rh"};console.error("Falha ao ler estrutura XLSX do RH",detail);setMessage("Não foi possível ler a estrutura da planilha. Verifique se o arquivo XLSX possui uma tabela válida na primeira aba.")}finally{setReading(false)}}
  function raw(row:unknown[],field:AddressField){const index=mapping[field];return index===undefined?null:row[index]}
  function value(row:unknown[],field:AddressField){return String(raw(row,field)??"").trim()}
  function process(){
    if(mapping.nome===undefined){setMessage("Mapeie o campo obrigatório Nome.");return}
    const source=rows.slice(1).filter(r=>r.some(Boolean));
    const duplicated=duplicateCpfRows(source.map(r=>({cpf:normalizeCpf(value(r,"cpf"))})));
    const result=source.map((r,index)=>{
      const nome=value(r,"nome"),estado=(value(r,"estado")||value(r,"uf")).toUpperCase(),cpfRaw=value(r,"cpf"),cpf=normalizeCpf(cpfRaw),birthRaw=raw(r,"data_nascimento"),birth=parseBirthDate(birthRaw);
      const match=matchCollaborator({nome,cpf},employees,obrasEmployees),employee=match.privateRecord,obras=match.obrasRecord;
      const strongSuggestions=strongAuxiliaryMatches({data_nascimento:birth,telefone:value(r,"telefone"),cidade:value(r,"cidade"),uf:estado},employees);
      const similarPrivate=possibleMatches(nome,employees),similarObras=possibleMatches(nome,obrasEmployees);
      const suggestions=buildCollaboratorSuggestions(match,similarPrivate,strongSuggestions,similarObras);
      let status:AddressPreview["status"]=match.status;
      let resolucaoManual=status==="possivel_duplicidade";
      let mensagem=match.reason==="nome_privado_exato"&&!employee?.cpf&&cpf?"Cadastro existente — dados RH serão acrescentados.":status==="atualizacao"?"Atualização de cadastro existente; células vazias preservarão valores atuais.":status==="conflito"?"Conflito de CPF — revisão manual necessária.":status==="novo_vinculado"?"Novo cadastro privado — vinculado ao Obras Control.":status==="novo_externo"?"Novo colaborador externo.":"Possível duplicidade — confirmar manualmente.";
      if(!nome||(cpfRaw&&!isValidCpf(cpfRaw))||(birthRaw!=null&&String(birthRaw).trim()&&!birth)||(estado&&!validUf(estado))){status="erro";resolucaoManual=false;mensagem="Nome, CPF, data de nascimento ou UF inválido."}
      else if(cpf&&duplicated.has(cpf)){status="possivel_duplicidade";resolucaoManual=false;mensagem="CPF duplicado dentro do arquivo — resolva antes de importar."}
      else if(status==="novo_externo"&&suggestions.length){status="possivel_duplicidade";resolucaoManual=true;mensagem=strongSuggestions.length?"Possível duplicidade — dados auxiliares coincidem; confirme manualmente.":"Possível duplicidade — nome semelhante exige confirmação manual."}
      return {linha:index+2,colaborador_id:employee?.id||null,funcionario_id:employee?.funcionario_id||obras?.id||null,nome,data_nascimento:birth,cpf,rg:value(r,"rg"),telefone:normalizePhone(value(r,"telefone")),logradouro:value(r,"logradouro"),bairro:value(r,"bairro"),cidade:value(r,"cidade"),estado,status,mensagem,sugestoes:suggestions,permite_externo:canKeepAsExternal(match)&&suggestions.length>0,resolucao_manual:resolucaoManual};
    });setPreview(result);setConfirmed(false);setMessage("")
  }
  function resolveSuggestion(linha:number,choice:string){setPreview(current=>current.map(item=>{if(item.linha!==linha)return item;const suggestion=choice==="__external__"?null:item.sugestoes.find(candidate=>`${candidate.origem}:${candidate.id}`===choice)||null;return resolveCollaboratorSuggestion(item,suggestion)}));setConfirmed(false)}
  async function save(){if(!confirmed)return;const {data,error}=await supabase.rpc("ro_importar_colaboradores_rh",{p_arquivo_nome:fileName,p_linhas:preview});setMessage(error?error.message:`Importação concluída: ${Number((data as {novos?:number})?.novos||0)} novo(s), ${Number((data as {atualizados?:number})?.atualizados||0)} atualizado(s), 0 funcionários criados no Obras Control.`);if(!error){setPreview([]);setRows([]);setConfirmed(false);load()}}
  function openEmployee(employee:AddressEmployee){setEditorMessage(null);setSelectedEmployee(employee);setEditingAddress({id:employee.id,funcionario_id:employee.funcionario_id||"",nome:employee.nome,data_nascimento:employee.data_nascimento||"",cpf:employee.cpf||"",rg:employee.rg||"",telefone:employee.telefone||"",logradouro:normalizeStreetForDisplay(employee.logradouro),bairro:normalizeNeighborhoodForDisplay(employee.bairro),cidadeUf:formatCityUf(employee.cidade,employee.uf),ativo:employee.ativo})}
  function closeEmployee(){setSelectedEmployee(null);setEditorMessage(null)}
  async function saveManual(){const parsed=parseCityUf(editingAddress.cidadeUf);if(!parsed.ok){setEditorMessage({kind:"error",text:parsed.reason==="invalid"?"Informe uma UF válida.":"Informe a cidade e a UF."});return}const cleaned={...editingAddress,cidadeUf:formatCityUf(parsed.cidade,parsed.uf),logradouro:normalizeStreetForDisplay(editingAddress.logradouro),bairro:normalizeNeighborhoodForDisplay(editingAddress.bairro)};if(!selectedEmployee||!cleaned.nome.trim()||(cleaned.cpf&&!isValidCpf(cleaned.cpf))){setEditorMessage({kind:"error",text:"Revise nome e CPF."});return}const payload={...cleaned,cidade:parsed.cidade,estado:parsed.uf};delete (payload as Partial<typeof payload>).cidadeUf;setSavingEmployee(true);setEditorMessage(null);const {error}=await supabase.rpc("ro_salvar_colaborador_viagem",{p_colaborador:payload});setSavingEmployee(false);if(error){setEditorMessage({kind:"error",text:error.message});return}setEditingAddress(cleaned);setEmployees(current=>current.map(employee=>employee.id===selectedEmployee.id?{...employee,...payload,funcionario_id:payload.funcionario_id||null,logradouro:payload.logradouro||null,bairro:payload.bairro||null,uf:payload.estado}:employee));setSelectedEmployee(current=>current?{...current,...payload,funcionario_id:payload.funcionario_id||null,logradouro:payload.logradouro||null,bairro:payload.bairro||null,uf:payload.estado}:current);setEditorMessage({kind:"success",text:"Colaborador salvo com segurança."})}
  const counts=(status:AddressPreview["status"])=>preview.filter(x=>x.status===status).length;
  const blocked=counts("erro")+counts("possivel_duplicidade")+counts("conflito");
  const headerRow=Array.isArray(rows[0])?rows[0]:[];
const content=<section className="card form"><h2>Cadastro privado de colaboradores para viagens</h2><p>O XLSX é processado localmente. Colaboradores externos são importados aqui e nunca criados no Obras Control.</p><label className="wide">Planilha completa do RH (.xlsx)<input type="file" disabled={reading} accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={e=>choose(e.target.files?.[0])}/></label>{reading&&<div className="alert wide" role="status">Lendo planilha...</div>}{rows.length>0&&<div className="wide"><h3>Mapeamento de colunas</h3><div className="grid two">{(Object.keys(addressLabels) as AddressField[]).map(field=><label key={field}>{addressLabels[field]}{field==="nome"?" *":""}<select value={mapping[field]??""} onChange={e=>setMapping({...mapping,[field]:e.target.value===""?undefined:Number(e.target.value)})}><option value="">Não mapear</option>{headerRow.map((h,i)=><option value={i} key={i}>{String(h||`Coluna ${i+1}`)}</option>)}</select></label>)}</div><button className="btn primary" disabled={reading} type="button" onClick={process}>Processar prévia</button></div>}{preview.length>0&&<div className="wide"><h3>Prévia obrigatória</h3><p>{preview.length} linhas lidas · {counts("atualizacao")} atualizações · {counts("novo_vinculado")} novos vinculados ao Obras · {counts("novo_externo")} externos · {blocked} pendências · 0 funcionários criados no Obras Control</p><div className="table-wrap"><table><thead><tr><th>Linha</th><th>Colaborador</th><th>CPF</th><th>Classificação</th><th>Cidade / UF</th></tr></thead><tbody>{preview.map(x=><tr key={x.linha}><td>{x.linha}</td><td>{x.nome}</td><td>{formatCpf(x.cpf)||"—"}</td><td><strong>{x.status}</strong><br/>{x.mensagem}{x.status==="possivel_duplicidade"&&x.resolucao_manual&&x.sugestoes.length>0&&<select aria-label={`Resolver pendência da linha ${x.linha}`} value="" onChange={e=>resolveSuggestion(x.linha,e.target.value)}><option value="">Selecione uma resolução</option>{x.sugestoes.map(suggestion=><option key={`${suggestion.origem}:${suggestion.id}`} value={`${suggestion.origem}:${suggestion.id}`}>{suggestion.nome} — {suggestion.origem==="obras"?"Obras Control":"Cadastro privado"}</option>)}{x.permite_externo&&<option value="__external__">Manter como novo colaborador externo</option>}</select>}</td><td>{x.cidade||"—"} / {x.estado||"—"}</td></tr>)}</tbody></table></div><label className="checkbox"><input type="checkbox" checked={confirmed} onChange={e=>setConfirmed(e.target.checked)}/> Confirmo a importação após revisar a prévia.</label><button className="btn primary" type="button" disabled={!confirmed||blocked>0} onClick={save}>Confirmar importação</button></div>}<div className="wide"><h3>Consulta e edição privada</h3><div className="actions"><input placeholder="Buscar por nome, CPF ou telefone" value={search} onChange={e=>setSearch(e.target.value)}/><button className="btn secondary" type="button" onClick={load}>Buscar</button></div>{employees.filter(employee=>!search||[employee.nome,employee.cpf||"",employee.telefone||""].some(value=>value.toLocaleLowerCase("pt-BR").includes(search.toLocaleLowerCase("pt-BR")))).map(employee=><div className={`collaborator-row${selectedEmployee?.id===employee.id?" editing":""}`} key={employee.id}><div className="actions"><span><strong>{employee.nome}</strong> — {employee.cpf?formatCpf(employee.cpf):"sem CPF"} · {employee.ativo?"Ativo":"Inativo"}</span><button className="btn secondary" type="button" aria-expanded={selectedEmployee?.id===employee.id} aria-controls={`editor-colaborador-${employee.id}`} onClick={()=>openEmployee(employee)}>Visualizar / Editar {employee.nome}</button></div>{selectedEmployee?.id===employee.id&&<div className="collaborator-inline-editor" id={`editor-colaborador-${employee.id}`}><h3>Dados do colaborador — {selectedEmployee.nome}</h3><div className="grid two">{(["nome","data_nascimento","cpf","rg","telefone","logradouro","bairro","cidadeUf"] as const).map(field=><label key={field}>{field==="cidadeUf"?"Cidade / UF":addressLabels[field as AddressField]||field}<input type={field==="data_nascimento"?"date":"text"} value={field==="cpf"?formatCpf(editingAddress[field]):field==="telefone"?formatPhone(editingAddress[field]):String(editingAddress[field])} onChange={e=>setEditingAddress({...editingAddress,[field]:field==="cpf"?normalizeCpf(e.target.value)||e.target.value:field==="telefone"?normalizePhone(e.target.value):e.target.value})}/></label>)}<label className="checkbox"><input type="checkbox" checked={editingAddress.ativo} onChange={e=>setEditingAddress({...editingAddress,ativo:e.target.checked})}/> Ativo</label></div>{editorMessage&&<div className={editorMessage.kind} role={editorMessage.kind==="error"?"alert":"status"}>{editorMessage.text}</div>}<div className="actions"><button className="btn primary" disabled={savingEmployee} type="button" onClick={saveManual}>{savingEmployee?"Salvando...":"Salvar colaborador"}</button><button className="btn secondary" disabled={savingEmployee} type="button" onClick={closeEmployee}>Fechar</button></div></div>}</div>)}</div>{message&&<div className="alert wide">{message}</div>}</section>;
  return embedded?content:<Page title="Importação" subtitle="Cadastro privado de colaboradores para viagens">{content}</Page>;
}
