...
import { normalizeNeighborhoodForDisplay, normalizeStreetForDisplay } from "./collaboratorDisplay";
...
export function EnderecosFuncionarios({embedded=false}:{embedded?:boolean}={}) {
  ...
  const [search,setSearch]=useState("");const [selectedEmployee,setSelectedEmployee]=useState<AddressEmployee|null>(null);const [editingAddress,setEditingAddress]=useState({id:"",funcionario_id:"",nome:"",data_nascimento:"",cpf:"",rg:"",telefone:"",logradouro:"",bairro:"",cidade:"",estado:"",ativo:true});
  ...
  function openEmployee(employee:AddressEmployee){setEditorMessage(null);setSelectedEmployee(employee);setEditingAddress({id:employee.id,funcionario_id:employee.funcionario_id||"",nome:employee.nome,data_nascimento:employee.data_nascimento||"",cpf:employee.cpf||"",rg:employee.rg||"",telefone:employee.telefone||"",logradouro:normalizeStreetForDisplay(employee.logradouro),bairro:normalizeNeighborhoodForDisplay(employee.bairro),cidade:employee.cidade||"",estado:employee.uf||"",ativo:employee.ativo})}
  function closeEmployee(){setSelectedEmployee(null);setEditorMessage(null)}
  async function saveManual(){const cleaned={...editingAddress,logradouro:normalizeStreetForDisplay(editingAddress.logradouro),bairro:normalizeNeighborhoodForDisplay(editingAddress.bairro)};if(!selectedEmployee||!cleaned.nome.trim()||(cleaned.cpf&&!isValidCpf(cleaned.cpf))||(cleaned.estado&&!validUf(cleaned.estado))){setEditorMessage({kind:"error",text:"Revise nome, CPF e UF."});return}setSavingEmployee(true);setEditorMessage(null);const {error}=await supabase.rpc("ro_salvar_colaborador_viagem",{p_colaborador:cleaned});setSavingEmployee(false);if(error){setEditorMessage({kind:"error",text:error.message});return}setEditingAddress(cleaned);setEmployees(current=>current.map(employee=>employee.id===selectedEmployee.id?{...employee,...cleaned,funcionario_id:cleaned.funcionario_id||null,logradouro:cleaned.logradouro||null,bairro:cleaned.bairro||null,uf:cleaned.estado||null}:employee));setSelectedEmployee(current=>current?{...current,...cleaned,funcionario_id:cleaned.funcionario_id||null,logradouro:cleaned.logradouro||null,bairro:cleaned.bairro||null,uf:cleaned.estado||null}:current);setEditorMessage({kind:"success",text:"Colaborador salvo com segurança."})}
