...
import { autoMapHeaders, duplicateCpfRows, formatCpf, formatPhone, isSpreadsheetRows, isValidCpf, matchCollaborator, MAX_RH_XLSX_BYTES, normalizeCpf, normalizePhone, parseBirthDate, possibleMatches, validUf, type AddressField, type ColumnMapping, type SpreadsheetRows } from "./addressImport";
import type {
  Anexo,
  Custo,
  DesligamentoSubtipo,
...
export function EnderecosFuncionarios({embedded=false}:{embedded?:boolean}={}) {
  ...
  function process(){if(mapping.nome===undefined){setMessage("Mapeie o campo obrigatório Nome.");return}const source=rows.slice(1).filter(r=>r.some(Boolean));const preliminary=source.map(r=>({cpf:normalizeCpf(value(r,"cpf"))}));const duplicated=duplicateCpfRows(preliminary);const result=source.map((r,index)=>{const nome=value(r,"nome"),estado=(value(r,"estado")||value(r,"uf")).toUpperCase(),cpfRaw=value(r,"cpf"),cpf=normalizeCpf(cpfRaw),birthRaw=raw(r,"data_nascimento"),birth=parseBirthDate(birthRaw),match=matchCollaborator({nome,cpf},employees,obrasEmployees),employee=match.privateRecord,obras=match.obrasRecord;let status:AddressPreview["status"]=match.status,mensagem=status==="atualizacao"?"Atualização de cadastro privado; células vazias preservarão valores atuais.":status==="novo_vinculado"?"Novo cadastro privado — vinculado ao Obras Control.":status==="novo_externo"?"Novo colaborador externo.":"Nome corresponde a mais de um cadastro. Selecione um funcionário do Obras ou mantenha como externo.";if(!nome||(cpfRaw&&!isValidCpf(cpfRaw))||(birthRaw!=null&&String(birthRaw).trim()&&!birth)||(estado&&!validUf(estado))){status="erro";mensagem="Nome, CPF, data de nascimento ou UF inválido."}else if(cpf&&duplicated.has(cpf)){status="possivel_duplicidade";mensagem="CPF duplicado dentro do arquivo — resolva antes de importar."}else if(status==="novo_externo"&&possibleMatches(nome,employees).length){status="possivel_duplicidade";mensagem="Há sugestão de cadastro privado semelhante; confirme manualmente antes de criar externo."}return {linha:index+2,colaborador_id:employee?.id||null,funcionario_id:employee?.funcionario_id||obras?.id||null,nome,data_nascimento:birth,cpf,rg:value(r,"rg"),telefone:normalizePhone(value(r,"telefone")),logradouro:value(r,"logradouro"),bairro:value(r,"bairro"),cidade:value(r,"cidade"),estado,status,mensagem,sugestoes:match.candidates.map(x=>x.id)}});setPreview(result);setConfirmed(false);setMessage("")}
  ...
  const blocked=counts("erro")+counts("possivel_duplicidade");
