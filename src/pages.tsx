...
import { autoMapHeaders, cpfMatches, duplicateCpfRows, formatCpf, formatPhone, isSpreadsheetRows, isValidCpf, MAX_RH_XLSX_BYTES, normalizeCpf, normalizePhone, parseBirthDate, possibleMatches, safeMatches, validUf, type AddressField, type ColumnMapping, type SpreadsheetRows } from "./addressImport";
import type {
  Anexo,
  Custo,
  DesligamentoSubtipo,
...
type AddressPreview={linha:number;colaborador_id:string|null;funcionario_id:string|null;nome:string;data_nascimento:string|null;cpf:string;rg:string;telefone:string;logradouro:string;bairro:string;cidade:string;estado:string;status:"novo"|"atualizacao"|"vinculado"|"possivel_duplicidade"|"erro"|"ignorado";mensagem:string;sugestoes:string[]};
...
export function EnderecosFuncionarios({embedded=false}:{embedded?:boolean}={}) {
  ...
  const [employees,setEmployees]=useState<AddressEmployee[]>([]); const [rows,setRows]=useState<SpreadsheetRows>([]);
  ...
  const load=useCallback(()=>supabase.rpc("ro_catalogo_colaboradores_privado",{p_busca:search||null}).then(({data,error})=>{if(error)setMessage(error.message);else setEmployees((data||[]) as AddressEmployee[])}),[search]);
  useEffect(()=>{load()},[load]);
  ...
  function process(){if(mapping.nome===undefined){setMessage("Mapeie o campo obrigatório Nome.");return}const source=rows.slice(1).filter(r=>r.some(Boolean));const preliminary=source.map(r=>({cpf:normalizeCpf(value(r,"cpf"))}));const duplicated=duplicateCpfRows(preliminary);const result=source.map((r,index)=>{const nome=value(r,"nome"),estado=(value(r,"estado")||value(r,"uf")).toUpperCase(),cpfRaw=value(r,"cpf"),cpf=normalizeCpf(cpfRaw),birthRaw=raw(r,"data_nascimento"),birth=parseBirthDate(birthRaw),byCpf=cpfMatches(cpf,employees),byName=safeMatches(nome,employees),employee=byCpf.length===1?byCpf[0]:byCpf.length===0&&byName.length===1?byName[0]:null,suggestions=employee?[]:possibleMatches(nome,employees);let status:AddressPreview["status"]=employee?"atualizacao":"novo",mensagem=employee?"Cadastro existente será atualizado; células vazias preservarão valores atuais.":"Novo colaborador sem vínculo com Obras Control.";if(!nome||(cpfRaw&&!isValidCpf(cpfRaw))||(birthRaw!=null&&String(birthRaw).trim()&&!birth)||(estado&&!validUf(estado))){status="erro";mensagem="Nome, CPF, data de nascimento ou UF inválido."}else if(cpf&&duplicated.has(cpf)){status="possivel_duplicidade";mensagem="CPF duplicado dentro do arquivo — resolva antes de importar."}else if(!employee&&(byName.length>1||suggestions.length)){status="possivel_duplicidade";mensagem="Correspondência ambígua — exige confirmação manual."}return {linha:index+2,colaborador_id:employee?.id||null,funcionario_id:employee?.funcionario_id||null,nome,data_nascimento:birth,cpf,rg:value(r,"rg"),telefone:normalizePhone(value(r,"telefone")),logradouro:value(r,"logradouro"),bairro:value(r,"bairro"),cidade:value(r,"cidade"),estado,status,mensagem,sugestoes:suggestions.map(x=>x.id)}});setPreview(result);setConfirmed(false);setMessage("")}
