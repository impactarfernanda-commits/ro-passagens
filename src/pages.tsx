...
import { autoMapHeaders, duplicateCpfRows, formatCpf, formatPhone, isSpreadsheetRows, isValidCpf, matchCollaborator, MAX_RH_XLSX_BYTES, normalizeCpf, normalizePhone, parseBirthDate, possibleMatches, strongAuxiliaryMatches, validUf, type AddressField, type ColumnMapping, type SpreadsheetRows } from "./addressImport";
import type {
  Anexo,
  Custo,
  DesligamentoSubtipo,
...
export function EnderecosFuncionarios({embedded=false}:{embedded?:boolean}={}) {
  ...
  function process(){
    ...
    const result=source.map((r,index)=>{
      ...
      if(!nome||(cpfRaw&&!isValidCpf(cpfRaw))||(birthRaw!=null&&String(birthRaw).trim()&&!birth)||(estado&&!validUf(estado))){status="erro";mensagem="Nome, CPF, data de nascimento ou UF inválido."}
      else if(cpf&&duplicated.has(cpf)){status="possivel_duplicidade";mensagem="CPF duplicado dentro do arquivo — resolva antes de importar."}
      else if(status==="novo_externo"&&(strongSuggestions.length||possibleMatches(nome,employees).length)){status="possivel_duplicidade";mensagem=strongSuggestions.length?"Possível duplicidade — dados auxiliares coincidem; confirme manualmente.":"Possível duplicidade — nome semelhante exige confirmação manual."}
      return {linha:index+2,colaborador_id:employee?.id||null,funcionario_id:employee?.funcionario_id||obras?.id||null,nome,data_nascimento:birth,cpf,rg:value(r,"rg"),telefone:normalizePhone(value(r,"telefone")),logradouro:value(r,"logradouro"),bairro:value(r,"bairro"),cidade:value(r,"cidade"),estado,status,mensagem,sugestoes:match.candidates.map(x=>x.id)};
    });setPreview(result);setConfirmed(false);setMessage("")
  }
  function resolveObrasMatch(linha:number,choice:string){const funcionarioId=choice==="__external__"?"":choice;setPreview(current=>current.map(item=>item.linha!==linha?item:{...item,funcionario_id:funcionarioId||null,status:funcionarioId?"novo_vinculado":"novo_externo",mensagem:funcionarioId?"Novo cadastro privado — vínculo com Obras confirmado manualmente.":"Novo colaborador externo — opção confirmada manualmente."}));setConfirmed(false)}
