...
export function EnderecosFuncionarios({embedded=false}:{embedded?:boolean}={}) {
  ...
  function openEmployee(employee:AddressEmployee){setSelectedEmployee(employee);setEditingAddress({id:employee.id,funcionario_id:employee.funcionario_id||"",nome:employee.nome,data_nascimento:employee.data_nascimento||"",cpf:employee.cpf||"",rg:employee.rg||"",telefone:employee.telefone||"",logradouro:employee.logradouro||"",bairro:employee.bairro||"",cidade:employee.cidade||"",estado:employee.uf||"",ativo:employee.ativo})}
  async function saveManual(){if(!selectedEmployee||!editingAddress.nome.trim()||(editingAddress.cpf&&!isValidCpf(editingAddress.cpf))||(editingAddress.estado&&!validUf(editingAddress.estado))){setMessage("Revise nome, CPF e UF.");return}const {error}=await supabase.rpc("ro_salvar_colaborador_viagem",{p_colaborador:editingAddress});setMessage(error?error.message:"Colaborador salvo com segurança.");if(!error){setSelectedEmployee(null);load()}}
