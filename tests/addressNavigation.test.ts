...
test("página independente amplia XLSX, prévia, matching e edição",()=>{const page=pages.slice(pages.indexOf("export function EnderecosFuncionarios"));for(const token of [".xlsx","Prévia obrigatória","matchCollaborator","Visualizar / Editar","ro_salvar_colaborador_viagem"])assert.ok(page.includes(token),token);assert.match(page,/title="Importação" subtitle="Cadastro privado de colaboradores para viagens"/)});
